...
class HiddenValue:
    ...
    def get_effect_for_level(self, level_idx: int) -> "LevelEffect":
        """
        获取指定档位索引的 LevelEffect。

        供 gms_tools.py（AgentScope ToolCall）调用，
        使 LLM 能在叙事过程中查询任意档位而非仅当前档的效果。

        Args:
            level_idx: 档位索引（0-based），超出范围时截断到边界

        Returns:
            该档位的 LevelEffect 实例
        """
        idx = max(0, min(level_idx, len(self.thresholds) - 1))
        threshold = self.thresholds[idx]
        return self.effects.get(threshold, LevelEffect())

    def get_locked_options_at_level(self, level_idx: int) -> List[str]:
        """
        获取指定档位及以下所有档位锁定的选项（累积）。

        与 get_locked_options() 相同，但接受任意档位索引而非仅当前档。
        """
        idx = max(0, min(level_idx, len(self.thresholds) - 1))
        locked: List[str] = []
