...
- 未弹出创建场景的对话框
- 与预览按钮问题类似，均属于编辑器交互无反馈问题

**问题分析：**
- `+` 按钮的点击事件可能未绑定或事件处理函数有错误
- 前端无错误提示，用户不知道操作是否被接受

**优先级：** P2（编辑器核心功能，创建场景是基本操作）

**建议：**
1. 检查 `+` 按钮的点击事件绑定和事件处理函数
2. 添加操作反馈（如加载状态、成功/失败提示）
3. 如果创建需要先选择剧本，应给出明确提示
...
- 仍显示场景标题和场景内容的表单字段
- 界面元素引用未更新，说明tab切换逻辑可能失效

**问题分析：**
- Tab切换的JavaScript逻辑可能有错误
- 可能与之前报告的"切换游戏后tab自动切换为角色"问题相关

**优先级：** P2（编辑器核心功能，tab切换是基本操作）

**建议：**
1. 检查tab切换的JavaScript事件处理
2. 验证tab状态管理是否正确（当前选中tab的样式变化）
3. 确保切换时正确加载对应面板内容
